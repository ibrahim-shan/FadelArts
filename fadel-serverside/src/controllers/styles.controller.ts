...
export const createStyle = asyncHandler(async (req: Request, res: Response) => {
  ...
  const doc = await Style.create({ name, description: description ?? '', slug, image: image ?? '' });
...
export const deleteStyle = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params as { id: string };
  const st = await Style.findById(id).lean();
  ...
  if (inUse > 0) {
    return res.status(409).json({ ok: false, error: 'Style is in use by products and cannot be deleted.' });
  }
